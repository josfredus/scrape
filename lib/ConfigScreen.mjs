...
export const ConfigScreen = function() {
  ...
  this._input.placeholder = 'Press Enter to show help'
...
ConfigScreen.prototype.getConfig = async function() {
  ...
  if (this._input.value === '' && !this._helpIsShown) {
    this._helpPanel.classList.remove('hidden')
    this._helpIsShown = true
    this._input.placeholder = 'Example: aww pics (top, month) / me_irl -sk 3'
    await this._listenForEnter()
  }
  this._input.blur()
  while (this._errorPanel.firstChild) {
    this._errorPanel.removeChild(this._errorPanel.firstChild)
  }
  this._ensureErrorPanelIsHidden()
...
ConfigScreen.prototype._ensureErrorPanelIsHidden = function() {
  if (this._errorIsShown) {
    this._errorPanel.classList.add('hidden')
    this._errorIsShown = false
  }
}

ConfigScreen.prototype._ensureErrorPanelIsShown = function() {
  if (!this._errorIsShown) {
    this._errorPanel.classList.remove('hidden')
